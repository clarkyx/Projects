...
public:
	...
	// getLight is used to calculate shadow effects
	double getlight(Ray3D& ray);
...
private:
	...
	// Allocates and initializes the pixel buffer for rendering, temp
	// buffer prepare for texture maping
	void inittemptextureBuffer();
	...
	int _scrWidth;
	int _scrHeight;
	// Level of anti alising
	int _anti;

	// enable texture or not 
	bool _texture = true;
	...
	unsigned char* _trbuffer;
	unsigned char* _tgbuffer;
	unsigned char* _tbbuffer;
	unsigned char* _srbuffer;
	unsigned char* _sgbuffer;
	unsigned char* _sbbuffer;
