...
//max ray counter on both reflection and refraction
#ifndef Maxref
#define Maxref (5)
#endif

//enable or disable reflection variable
#define RFAPPLY
//enable or disable refraction variable
#define RFAAPPLY
//#define NOSHADOW
...
struct Material {
	Material( Colour ambient, Colour diffuse, Colour specular, double exp ) :
		...
		specular_exp(exp), refint(0.05), tint(0.0), cs(1.0){}
	Material( Colour ambient, Colour diffuse, Colour specular, double exp, double ref) :
		ambient(ambient), diffuse(diffuse), specular(specular), 
		specular_exp(exp), refint(ref),tint(0.0), cs(1.0){}
	Material( Colour ambient, Colour diffuse, Colour specular, double exp, double ref, 
			  double refa, double c) :
		ambient(ambient), diffuse(diffuse), specular(specular), 
		specular_exp(exp), refint(ref),tint(refa), cs(c){}
...
// Ray structure. 
struct Ray3D {
	Ray3D() : ref(0),refa(0),c(1.0){
		intersection.none = true; 
	}
	Ray3D( Point3D p, Vector3D v ) : origin(p), dir(v), ref(0),refa(0),c(1.0){
		intersection.none = true;
	}
	Ray3D(Point3D p, Vector3D v, int refcount) : origin(p), dir(v), ref(refcount),refa(0),c(1.0){
		intersection.none = true;
	}
	Ray3D(Point3D p, Vector3D v, int refcount, int refacount, double ls) : origin(p), dir(v), ref(refcount),refa(refacount),c(ls){
		intersection.none = true;
	}
	...
	// function.
	Colour col;
	//reflection counters, must less than Maxref ref in order to
	//prevent heavy computation time
	int ref;
	//refraction counters, must less than Maxref 
	int refa;
	//light speed on differnt materials
	double c;
	//interpolation time: tried to implement motion blur
	double time;
};
#endif
